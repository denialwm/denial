...
void main() {
  test('does not repeat the bundled default from the local library', () async {
    ...
    final bundledDefaultName = p.basename(defaultShellWallpaperAsset);
    await File(
      p.join(directory.path, bundledDefaultName),
    ).writeAsBytes(<int>[1]);
    await File(
      p.join(directory.path, 'another-wallpaper.png'),
    ).writeAsBytes(<int>[2]);
    ...
    expect(
      page.items.where(
        (item) =>
            ...
            p.basenameWithoutExtension(bundledDefaultName).toLowerCase(),
      ),
      isEmpty,
    );
