...
class NotificationTestClient {
  ...
  Future<int> sendMarkup() {
    return notify(
      NotificationRequest(
        ...
        body: 'This has <b>bold</b>, <i>italic</i>, <u>underline</u>, a '
  ...
  Future<void> _waitForInteraction(int id) async {
    ...
    try {
      final interaction =
          await result.future.timeout(const Duration(minutes: 2));
      ...
      stderr
          .writeln('Timed out waiting for interaction with #$id; closing it.');
      await closeNotification(id);
    } finally {
