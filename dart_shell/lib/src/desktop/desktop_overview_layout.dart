...
abstract final class DesktopOverviewLayout {
  ...
  static void _sortSpatially(List<DesktopOverviewItem> items, Rect available) {
    ...
    items.sort((left, right) {
      final leftBand = ((left.frame.center.dy - available.top) / rowBand)
          .floor();
      final rightBand = ((right.frame.center.dy - available.top) / rowBand)
          .floor();
      final bandOrder = leftBand.compareTo(rightBand);
      if (bandOrder != 0) {
        return bandOrder;
      }
      final horizontalOrder = left.frame.center.dx.compareTo(
        right.frame.center.dx,
      );
      if (horizontalOrder != 0) {
        return horizontalOrder;
      }
      final verticalOrder = left.frame.center.dy.compareTo(
        right.frame.center.dy,
      );
      if (verticalOrder != 0) {
  ...
  static _OverviewCandidate? _layoutRows(
    ...
    final naturalHeightSum = naturalHeights.fold<double>(
      0.0,
    ...
    final movementScale = math.max(
      1.0,
      available.width * available.width + available.height * available.height,
    );
    final normalizedMovement = movement / movementScale;
    final score =
        occupiedArea * (0.88 + 0.12 * balance) -
  ...
  static double _sourcePreviewHeight(DesktopOverviewItem item) {
    ...
    return math.min(item.frame.height, item.frame.width / item.aspectRatio);
  }
}
