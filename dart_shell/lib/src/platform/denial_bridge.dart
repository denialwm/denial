...
class DenialBridge {
  ...
  static final Uint8List _hapticPrewarmPayload =
      Uint8List.fromList(const <int>[0]);
  static final Uint8List _hapticTapPayload = Uint8List.fromList(const <int>[1]);
  static final ByteData _hapticPrewarmData =
      ByteData.sublistView(_hapticPrewarmPayload);
  static final ByteData _hapticTapData =
      ByteData.sublistView(_hapticTapPayload);
  ...
  Future<DisplayLayout?> getDisplayLayout() {
    ...
    _sendWire(_wireCodec.encodeWindowRequest(
  ...
  void setBrightness(double level) {
    final data = ByteData(8)
      ..setFloat64(0, level.clamp(0.0, 1.0).toDouble(), Endian.little);
    ServicesBinding.instance.defaultBinaryMessenger
  ...
  Future<ByteData?> _handleAudioStateMessage(ByteData? data) async {
    ...
    final requestSerial =
        data.lengthInBytes >= 5 ? data.getUint32(1, Endian.little) : 0;
    final completesRead = _pendingAudioReads.isNotEmpty;
    if (!_audioStates.isClosed) {
      _audioStates.add(DenialAudioState(
        level: level,
  ...
  Future<ByteData?> _handleBrightnessStateMessage(ByteData? data) async {
    ...
    _brightnessStates.add(DenialBrightnessState(
      monitorId: monitorId,
    ...
    try {
      ...
      if (payload is wire.WindowSnapshot) {
        ...
        if (!_shellActions.isClosed) {
          _shellActions.add(DenialShellActionEvent(
            action: action,
  ...
  void _completeDisplayLayout(
    int requestId,
    wire.DisplayLayout payload,
  ) {
