      ...
      child: ExcludeSemantics(
        child: RepaintBoundary(
          child: SizedBox(
            ...
            height: 92,
            child: CustomPaint(
              painter: _ShellFrameTimePainter(_stats),
            ),
          ),
        ),
      ),
...
class _ImportedFrameTimePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _paintText(
      canvas,
      hasSamples ? '~${stats.frameMs.toStringAsFixed(1)} ms' : '--.- ms',
    ...
    _paintText(
      canvas,
      hasSamples
          ? 'AVG ${stats.averageMs.toStringAsFixed(1)}  MAX ${stats.maxMs.toStringAsFixed(1)}  OVER ${stats.overBudgetFrames}  N ${stats.sampleCount}'
          : 'AVG --.-  MAX --.-  OVER -  N -',
      const Offset(10, 28),
      _metricStyle,
  ...
  bool shouldRepaint(covariant _ImportedFrameTimePainter oldDelegate) {
    return oldDelegate.title != title || oldDelegate.stats != stats;
  }
}
...
class _ShellFrameTimePainter extends CustomPainter {
  const _ShellFrameTimePainter(this.stats);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _paintText(
      canvas,
      hasSamples
          ? 'SHELL · ${stats.refreshRate.round()} HZ · RENDER'
          : 'SHELL · WAIT',
      const Offset(10, 9),
      _labelStyle,
    );
    ...
    _paintText(
      canvas,
      'UI ${stats.buildMs.toStringAsFixed(1)}  R ${stats.rasterMs.toStringAsFixed(1)}  GAP ${gap == null ? '--.-' : gap.toStringAsFixed(1)}',
      const Offset(10, 28),
      _metricStyle,
    );
    _paintText(
      canvas,
      'AVG ${stats.averageMs.toStringAsFixed(1)}  MAX ${stats.maxMs.toStringAsFixed(1)}  OVER ${stats.overBudgetFrames}',
      const Offset(10, 42),
      _labelStyle,
    );
