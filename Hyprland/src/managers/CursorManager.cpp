...
CCursorManager::CCursorManager() {
    ...
    if (m_hyprcursor->valid() && *PUSEHYPRCURSOR) {
        ...
        const auto size = Env::get("HYPRCURSOR_SIZE");
        if (size) {
            try {
        ...
        const auto size = Env::get("XCURSOR_SIZE");
        if (size) {
            try {
                m_size = std::stoi(*size);
    ...
    m_xcursor->loadTheme(Env::get("XCURSOR_THEME").value_or("default"), m_size, m_cursorScale);
...
bool CCursorManager::changeTheme(const std::string& name, const int size) {
    ...
    const auto xcursorTheme    = Env::get("XCURSOR_THEME").value_or("default");
    ...
    if (*PUSEHYPRCURSOR) {
        ...
        if (!m_hyprcursor->valid()) {
            Log::logger->log(Log::ERR, "Hyprcursor failed loading theme \"{}\", falling back to XCursor.", m_theme);
            m_xcursor->loadTheme(m_theme.empty() ? xcursorTheme : m_theme, m_size, m_cursorScale);
        }
    } else
        m_xcursor->loadTheme(m_theme.empty() ? xcursorTheme : m_theme, m_size, m_cursorScale);
