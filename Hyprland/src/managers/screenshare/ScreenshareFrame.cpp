...
void CScreenshareFrame::renderMonitor() {
    ...
    if (Denial::renderLatestOutputFrame(PMONITOR, m_session->m_captureBox, m_overlayCursor))
        return;
...
void CScreenshareFrame::render() {
    ...
    const bool sessionLocked = [] {
        ...
        return router && router->secureSessionLocked();
    }();
    ...
    g_pHyprRenderer->startRenderPass();
    if (sessionLocked || PERM == PERMISSION_RULE_ALLOW_MODE_DENY || windowShareDenied) {
