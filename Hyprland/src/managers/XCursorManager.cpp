...
CXCursorManager::CXCursorManager() {
    if (const auto path = XcursorLibraryPath())
        m_libraryPath = path;
    m_home = Env::get("HOME").value_or("");
...
std::set<std::string> CXCursorManager::themePaths(std::string const& theme) {
    const auto& path = m_libraryPath;

    auto        expandTilde = [this](std::string const& path) {
        if (!path.empty() && path[0] == '~') {
            if (!m_home.empty())
                return m_home + path.substr(1);
        }
        return path;
    };
