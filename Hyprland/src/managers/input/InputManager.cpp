...
static std::optional<Denial::EFlutterPointerButton> flutterPointerButton(uint32_t button) {
    switch (button) {
        case BTN_LEFT: return Denial::EFlutterPointerButton::Primary;
        case BTN_RIGHT: return Denial::EFlutterPointerButton::Secondary;
        case BTN_MIDDLE: return Denial::EFlutterPointerButton::Middle;
        case BTN_SIDE:
        case BTN_BACK: return Denial::EFlutterPointerButton::Back;
        case BTN_EXTRA:
        case BTN_FORWARD: return Denial::EFlutterPointerButton::Forward;
        default: return std::nullopt;
    }
}

static Vector2D mapDenialPointerLocal(const Vector2D& outputLogical, const Vector2D& rectPos, const Vector2D& rectSize, const Vector2D& sourcePos, const Vector2D& sourceSize) {
    if (rectSize.x <= 0 || rectSize.y <= 0)
        return sourcePos;

    return {
        sourcePos.x + ((outputLogical.x - rectPos.x) * sourceSize.x / rectSize.x),
        sourcePos.y + ((outputLogical.y - rectPos.y) * sourceSize.y / rectSize.y),
    };
}

CInputManager::CInputManager() {
    m_denialRepeatTimer = makeShared<CEventLoopTimer>(
        std::nullopt,
        [this](SP<CEventLoopTimer> self, void*) {
            const auto  keyboard = m_denialRepeatKeyboard.lock();
            auto* const router   = Denial::inputRouter();
            const auto  owned    = keyboard && m_denialKeyboardOwnership.owns(keyboard.get(), m_denialRepeatKeycode, true);
            if (!keyboard || !owned || !router || !router->flutterKeyboardCapture() || keyboard->m_repeatRate <= 0 ||
                !router->sendFlutterKeyboardKey(Denial::SFlutterKeyboardEvent{
                    .timeMs  = sc<uint32_t>(Time::millis(Time::steadyNow())),
                    .keycode = m_denialRepeatKeycode,
                    .pressed = true,
                    .repeat  = true,
                })) {
                disarmDenialKeyboardRepeat();
                return;
            }
    ...
    if (g_pEventLoopManager)
        g_pEventLoopManager->addTimer(m_denialRepeatTimer);
...
CInputManager::~CInputManager() {
    if (m_denialRepeatTimer && g_pEventLoopManager) {
        g_pEventLoopManager->removeTimer(m_denialRepeatTimer);
        m_denialRepeatTimer.reset();
    }
    m_constraints.clear();
    m_keyboards.clear();
    m_pointers.clear();
...
void CInputManager::onMouseMoved(IPointer::SMotionEvent e) {
    ...
    const auto* const denialRouter = Denial::inputRouter();
    if (!denialRouter || !denialRouter->secureSessionLocked())
...
void CInputManager::sendMotionEventsToFocused() {
    ...
    // Keyboard focus changes must not steal pointer focus from the Flutter
    // shell. In particular, closing a window focuses the next client and
    // calls this helper; assigning that client pointer focus here lets its
    // cursor request overwrite the cursor already resolved by Flutter.
    if (m_denialPointer.target == EDenialPointerTarget::FlutterShell)
        return;
...
void CInputManager::onDenialSessionLockChanged(bool locked) {
    // A secure-boundary transition can happen while the key that initiated it
    // is still physically held. Balance Flutter's key-downs before its lock
    // widgets change, but retain ownership until the real releases arrive so
    // they cannot leak into compositor keybindings or the newly focused client.
    flushDenialKeyboardOwnership();
    ...
    // Tear down every client-owned capture before the lock frame becomes the
    // sole input target. A later release must not resurrect an old pointer,
    // touch, keyboard, DnD, constraint, or seat-grab route.
    if (PROTO::data)
    ...
    } else if (g_pSeatManager) {
        g_pSeatManager->sendTouchCancel();
    }
    m_touchData = {};

    clearDenialPointerTarget();
    g_pTrackpadGestures->cancelActiveGesture();
    g_pUnifiedWorkspaceSwipe->cancel();

    if (PROTO::tablet) {
        for (const auto& tool : m_tabletTools) {
            if (!tool)
                continue;
            if (tool->getSurface()) {
                if (tool->m_isDown)
                    PROTO::tablet->up(tool);
                for (const auto button : tool->m_buttonsDown)
                    PROTO::tablet->buttonTool(tool, button, false);
                PROTO::tablet->proximityOut(tool);
            }
            tool->setSurface(nullptr);
            tool->m_isDown = false;
            tool->m_buttonsDown.clear();
        }
    }
...
void CInputManager::resetDenialPointerTarget() {
    m_denialPointer.target = EDenialPointerTarget::None;
    m_denialPointer.monitor.reset();
    m_denialPointer.window.reset();
    m_denialPointer.surface.reset();
    m_denialPointer.rectPos       = {};
    m_denialPointer.rectSize      = {};
    m_denialPointer.sourcePos     = {};
    m_denialPointer.sourceSize    = {};
    m_denialPointer.outputLogical = {};
    m_denialPointer.local         = {};
    m_denialPointer.capturedButtons.clear();
}

void CInputManager::clearDenialPointerTarget() {
    if (m_denialPointer.target != EDenialPointerTarget::None) {
        if (auto* const ROUTER = Denial::inputRouter())
            ROUTER->sendFlutterPointerLeave();
    }

    resetDenialPointerTarget();
}

bool CInputManager::routeDenialPointerMotion(uint32_t timeMs, PHLMONITOR monitor, const Vector2D& global, bool capturedOnly) {
    auto* const ROUTER = Denial::inputRouter();
    if (!ROUTER) {
        clearDenialPointerTarget();
        return false;
    ...
    if (ROUTER->secureSessionLocked()) {
        if (!monitor || !monitor->m_output || !monitor->m_enabled)
            return true;
        if (m_denialPointer.target != EDenialPointerTarget::FlutterShell)
            resetDenialPointerTarget();
        m_denialPointer.target        = EDenialPointerTarget::FlutterShell;
        m_denialPointer.monitor       = monitor;
        m_denialPointer.outputLogical = global - monitor->m_position;
        g_pSeatManager->setPointerFocus(nullptr, {});
        ROUTER->sendFlutterPointerMove(monitor->m_id, m_denialPointer.outputLogical);
        return true;
    }

    const bool CAPTURED = !m_denialPointer.capturedButtons.empty();
    if (capturedOnly && !CAPTURED)
        return false;

    auto targetMonitor = CAPTURED ? m_denialPointer.monitor.lock() : monitor;
    if (!targetMonitor || !targetMonitor->m_output || !targetMonitor->m_enabled) {
        clearDenialPointerTarget();
        return false;
    }

    const Vector2D OUTPUT_LOGICAL = global - targetMonitor->m_position;
    m_denialPointer.outputLogical = OUTPUT_LOGICAL;

    if (CAPTURED) {
        if (m_denialPointer.target == EDenialPointerTarget::FlutterShell) {
            g_pSeatManager->setPointerFocus(nullptr, {});
            ROUTER->sendFlutterPointerMove(targetMonitor->m_id, OUTPUT_LOGICAL);
            return true;
        }

        const auto SURFACE = m_denialPointer.surface.lock();
        const auto WINDOW  = m_denialPointer.window.lock();
        if (m_denialPointer.target != EDenialPointerTarget::ClientWindow || !SURFACE || !validMapped(WINDOW)) {
            clearDenialPointerTarget();
            return false;
        }

        const auto LOCAL      = mapDenialPointerLocal(OUTPUT_LOGICAL, m_denialPointer.rectPos, m_denialPointer.rectSize, m_denialPointer.sourcePos, m_denialPointer.sourceSize);
        m_denialPointer.local = LOCAL;
    ...
    if (!ROUTER->hitTest(targetMonitor->m_id, OUTPUT_LOGICAL, hit)) {
        clearDenialPointerTarget();
        return false;
    }

    if (hit.kind == Denial::EInputHitKind::FlutterShell) {
        const bool ENTERING = m_denialPointer.target != EDenialPointerTarget::FlutterShell;
        // Flutter observes motion over both shell UI and embedded clients. Only
        // retarget native dispatch here; this pointer has not left its scene.
        if (ENTERING)
            resetDenialPointerTarget();

        m_denialPointer.target        = EDenialPointerTarget::FlutterShell;
        m_denialPointer.monitor       = targetMonitor;
        m_denialPointer.outputLogical = OUTPUT_LOGICAL;
        m_lastFocusOnLS               = false;
        g_pSeatManager->setPointerFocus(nullptr, {});

        if (ENTERING && cursorImageUnlocked())
    ...
    if (hit.kind != Denial::EInputHitKind::ClientWindow || !hit.surface || !validMapped(hit.window)) {
        clearDenialPointerTarget();
        return false;
    }

    // Keep the Flutter pointer stream continuous across the inverse handoff.
    if (m_denialPointer.target == EDenialPointerTarget::FlutterShell)
        resetDenialPointerTarget();

    m_denialPointer.target        = EDenialPointerTarget::ClientWindow;
    m_denialPointer.monitor       = targetMonitor;
    m_denialPointer.window        = hit.window;
    m_denialPointer.surface       = hit.surface;
    m_denialPointer.rectPos       = {hit.rect.x, hit.rect.y};
    m_denialPointer.rectSize      = {hit.rect.w, hit.rect.h};
    m_denialPointer.sourcePos     = {hit.sourceRect.x, hit.sourceRect.y};
    m_denialPointer.sourceSize    = {hit.sourceRect.w, hit.sourceRect.h};
    m_denialPointer.outputLogical = OUTPUT_LOGICAL;
    m_denialPointer.local         = hit.local;
    m_lastFocusOnLS               = false;
    m_emptyFocusCursorSet         = false;

    g_pSeatManager->setPointerFocus(hit.surface, hit.local);
    g_pSeatManager->sendPointerMotion(timeMs, hit.local);
    ROUTER->sendFlutterPointerMove(targetMonitor->m_id, OUTPUT_LOGICAL);
    return true;
}

bool CInputManager::routeDenialPointerButton(const IPointer::SButtonEvent& e, SP<IPointer> mouse) {
    if (g_pSessionLockManager->isSessionLocked() || (PROTO::data && PROTO::data->dndActive()))
        return false;

    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        auto monitor = m_denialPointer.monitor.lock();
        if (!monitor && g_pCompositor) {
            monitor = g_pCompositor->getMonitorFromVector(getMouseCoordsInternal());
            if (monitor) {
                m_denialPointer.target        = EDenialPointerTarget::FlutterShell;
                m_denialPointer.monitor       = monitor;
                m_denialPointer.outputLogical = getMouseCoordsInternal() - monitor->m_position;
            }
        }
        const auto button = flutterPointerButton(e.button);
        if (monitor && button) {
            if (e.state == WL_POINTER_BUTTON_STATE_PRESSED)
                router->sendFlutterPointerDown(monitor->m_id, m_denialPointer.outputLogical, *button);
            else
                router->sendFlutterPointerUp(monitor->m_id, m_denialPointer.outputLogical, *button);
        }
        return true;
    }

    const auto CAPTURED_BUTTON = std::ranges::find(m_denialPointer.capturedButtons, e.button) != m_denialPointer.capturedButtons.end();
    if (g_pSeatManager->m_seatGrab) {
        if (e.state == WL_POINTER_BUTTON_STATE_RELEASED && CAPTURED_BUTTON)
            std::erase(m_denialPointer.capturedButtons, e.button);
        return false;
    }
    ...
    const bool  PASS       = g_pKeybindManager->onMouseEvent(e, mouse);
    if (!PASS && !*PPASSMOUSE && !CAPTURED_BUTTON)
        return true;
    ...
    if (e.state == WL_POINTER_BUTTON_STATE_PRESSED) {
        if (m_denialPointer.target == EDenialPointerTarget::FlutterShell) {
            if (BUTTON) {
                if (auto* const ROUTER = Denial::inputRouter())
                    sent = ROUTER->sendFlutterPointerDown(monitor->m_id, m_denialPointer.outputLogical, *BUTTON);
            }
        } else {
            const auto SURFACE = m_denialPointer.surface.lock();
            const auto WINDOW  = m_denialPointer.window.lock();
            if (!SURFACE || !validMapped(WINDOW)) {
                clearDenialPointerTarget();
                return false;
            }

            Desktop::focusState()->rawWindowFocus(WINDOW, Desktop::FOCUS_REASON_CLICK, SURFACE);
            // A button event uses the last pointer position delivered by enter
            // or motion. Sending another absolute motion here can turn a
            // stationary click into a large move when the compositor cursor
            // was warped without moving the Flutter-rendered cursor.
            g_pSeatManager->setPointerFocus(SURFACE, m_denialPointer.local);
            g_pSeatManager->sendPointerButton(e.timeMs, e.button, e.state);
            g_pCompositor->changeWindowZOrder(WINDOW, true);
            if (auto* const ROUTER = Denial::inputRouter())
                ROUTER->notifyClientWindowActivated(WINDOW->m_stableID);
            sent = true;
        }

        if (sent && !CAPTURED_BUTTON)
            m_denialPointer.capturedButtons.push_back(e.button);
        return true;
    }
    ...
    std::erase(m_denialPointer.capturedButtons, e.button);
    // Ending capture must not manufacture pointer motion. The client may have
    // warped the compositor cursor while it was captured; the next physical
    // motion will re-run hit testing and retarget naturally.
    return true;
}

void CInputManager::dispatchPointerAxis(const IPointer::SAxisEvent& e, double factor, wl_pointer_axis_relative_direction relativeDirection) {
    ...
    if (e.source == WL_POINTER_AXIS_SOURCE_WHEEL && ((*PEMULATEDISCRETE >= 1 && std::abs(e.deltaDiscrete) != 120) || *PEMULATEDISCRETE >= 2)) {
        ...
        if (std::signbit(e.deltaDiscrete) != m_scrollWheelState.lastEventSign || e.axis != m_scrollWheelState.lastEventAxis ||
            e.timeMs - m_scrollWheelState.lastEventTime > 500 /* libinput's default timeout */) {
            m_scrollWheelState.accumulatedScroll = 0;
            // Send one discrete step on the first event for responsiveness.
            discrete = std::copysign(1, e.deltaDiscrete);
        } else
            discrete = 0;

        for (int accumulated = m_scrollWheelState.accumulatedScroll; accumulated >= interval; accumulated -= interval) {
            discrete += std::copysign(1, e.deltaDiscrete);
    ...
    const int32_t deltaDiscrete = std::abs(discrete) != 0 && std::abs(discrete) < 1 ? std::copysign(1, discrete) : std::round(discrete);

    g_pSeatManager->sendPointerAxis(e.timeMs, e.axis, delta, deltaDiscrete, value120, e.source, relativeDirection);
...
bool CInputManager::routeDenialPointerAxis(const IPointer::SAxisEvent& e, double factor, bool touchpadScroll) {
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        auto monitor = m_denialPointer.monitor.lock();
        if (!monitor && g_pCompositor) {
            monitor = g_pCompositor->getMonitorFromVector(getMouseCoordsInternal());
        ...
        if (monitor) {
            const Vector2D delta = e.axis == WL_POINTER_AXIS_VERTICAL_SCROLL ? Vector2D{0.0, e.delta * factor} : Vector2D{e.delta * factor, 0.0};
            router->sendFlutterPointerScroll(monitor->m_id, m_denialPointer.outputLogical, delta);
        }
        return true;
    }

    if (g_pSessionLockManager->isSessionLocked() || (PROTO::data && PROTO::data->dndActive()) || g_pSeatManager->m_seatGrab || m_denialPointer.target == EDenialPointerTarget::None)
        return false;
    ...
    if (m_denialPointer.target == EDenialPointerTarget::FlutterShell) {
        ...
        const Vector2D DELTA = e.axis == WL_POINTER_AXIS_VERTICAL_SCROLL ? Vector2D{0.0, e.delta * factor} : Vector2D{e.delta * factor, 0.0};
        return ROUTER->sendFlutterPointerScroll(monitor->m_id, m_denialPointer.outputLogical, DELTA);
    }
    ...
    if (!touchpadScroll && WINDOW->isScrollMouseOverridden())
        factor = WINDOW->getScrollMouse();
    else if (touchpadScroll && WINDOW->isScrollTouchpadOverridden())
        factor = WINDOW->getScrollTouchpad();

    // A wheel event uses the location from the last real enter or motion.
    // Recomputing it from the compositor cursor and manufacturing a motion
    // here makes every detent retarget XWayland and resets scroll state in
    // clients that react to pointer motion.
    g_pSeatManager->setPointerFocus(SURFACE, m_denialPointer.local);

    dispatchPointerAxis(e, factor, e.relativeDirection);
    return true;
}
...
void CInputManager::mouseMoveUnified(uint32_t time, bool refocus, bool mouse, std::optional<Vector2D> overridePos) {
    ...
    // Native lock ownership is the first input boundary. Do not expose locked
    // pointer motion to compositor plugins, constraints, grabs, or clients.
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        m_lastCursorPosFloored = MOUSECOORDSFLOORED;
        const auto monitor     = g_pCompositor->getMonitorFromVector(mouseCoords);
        if (monitor)
            routeDenialPointerMotion(time, monitor, mouseCoords);
        return;
    }
    ...
    const bool DND_ACTIVE = PROTO::data && PROTO::data->dndActive();
    if (g_pSeatManager->m_seatGrab || DND_ACTIVE) {
        if (auto* const ROUTER = Denial::inputRouter())
            ROUTER->notifyCursorPosition(PMONITOR->m_id, mouseCoords - PMONITOR->m_position);
    }
    ...
    if (!PROTO::data->dndActive() && !g_pSeatManager->m_seatGrab && !m_denialPointer.capturedButtons.empty() && routeDenialPointerMotion(time, PMONITOR, mouseCoords, true))
        return;
    ...
    if (!foundSurface && !PROTO::data->dndActive() && !g_pSeatManager->m_seatGrab) {
        if (routeDenialPointerMotion(time, PMONITOR, mouseCoords))
            return;
    } else if (foundSurface) {
        if (m_denialPointer.capturedButtons.empty() && m_denialPointer.target != EDenialPointerTarget::None)
            clearDenialPointerTarget();

        // Native surfaces still take pointer focus and receive their normal
        // Wayland motion events. Keep the Flutter-owned software cursor in
        // sync after that handoff, too. Seat grabs are already covered above.
        if (!g_pSeatManager->m_seatGrab && !DND_ACTIVE) {
            if (auto* const ROUTER = Denial::inputRouter())
                ROUTER->notifyCursorPosition(PMONITOR->m_id, mouseCoords - PMONITOR->m_position);
        }
...
void CInputManager::onMouseButton(IPointer::SButtonEvent e, SP<IPointer> mouse) {
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        routeDenialPointerButton(e, mouse);
        g_pSeatManager->sendPointerFrame();
        return;
    }
    ...
    switch (m_clickBehavior) {
        case CLICKMODE_DEFAULT:
            if (!routeDenialPointerButton(e, mouse))
                processMouseDownNormal(e, mouse);
            break;
...
void CInputManager::onMouseWheel(IPointer::SAxisEvent e, SP<IPointer> pointer) {
    ...
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        routeDenialPointerAxis(e, factor, ISTOUCHPADSCROLL);
        return;
    }
    ...
    if (routeDenialPointerAxis(e, factor, ISTOUCHPADSCROLL))
        return;
    ...
    dispatchPointerAxis(e, factor, WL_POINTER_AXIS_RELATIVE_DIRECTION_IDENTICAL);
}
...
void CInputManager::setupKeyboard(SP<IKeyboard> keeb) {
    ...
    keeb->m_keyboardEvents.keymap.listenStatic([this, keeb = keeb.get()] {
        auto       PKEEB  = keeb->m_self.lock();
        const auto LAYOUT = PKEEB->getActiveLayout();

        publishDenialKeyboardState(PKEEB);
    ...
    publishDenialKeyboardState(keeb);
...
void CInputManager::destroyKeyboard(SP<IKeyboard> pKeyboard) {
    ...
    m_denialKeyboardOwnership.eraseKeyboard(pKeyboard.get());
    if (m_denialRepeatKeyboard.lock() == pKeyboard)
        disarmDenialKeyboardRepeat();
...
bool CInputManager::routeDenialKeyboardKey(const IKeyboard::SKeyEvent& event, SP<IKeyboard> keyboard, bool allowNewCapture) {
    if (!keyboard || keyboard->isVirtual())
        return false;

    const bool  pressed        = event.state == WL_KEYBOARD_KEY_STATE_PRESSED;
    const bool  flutterOwnsKey = m_denialKeyboardOwnership.owns(keyboard.get(), event.keycode);
    auto* const router         = Denial::inputRouter();
    const bool  secureLocked   = router && router->secureSessionLocked();

    if (!pressed && flutterOwnsKey) {
        m_denialKeyboardOwnership.consumeRelease(keyboard.get(), event.keycode, [&](uint32_t keycode) {
            return router &&
                router->sendFlutterKeyboardKey(Denial::SFlutterKeyboardEvent{
                    .timeMs  = event.timeMs,
                    .keycode = keycode,
        ...
        if (m_denialRepeatKeyboard.lock() == keyboard && m_denialRepeatKeycode == event.keycode)
            disarmDenialKeyboardRepeat();

        // Never leak an orphan release to a Wayland client, even if capture or
        // the embedded engine disappeared after Flutter received the press.
        return true;
    }
    ...
    if (secureLocked) {
        if (pressed && !flutterOwnsKey &&
            ...
            m_denialKeyboardOwnership.acquire(keyboard.get(), event.keycode);
            armDenialKeyboardRepeat(keyboard, event.keycode);
        }
        // A missing/restarting Flutter isolate must fail closed: native lock
        // ownership is sufficient to consume the event.
        return true;
    }

    if (!pressed || !allowNewCapture || flutterOwnsKey || !router || !router->flutterKeyboardCapture())
        return flutterOwnsKey;

    // A key pressed before shell capture belongs to the focused client. Let
    // its release follow the same route so neither side gets a stuck key.
    if (std::ranges::contains(m_pressed, event.keycode))
        return false;

    if (!router->sendFlutterKeyboardKey(Denial::SFlutterKeyboardEvent{
            .timeMs  = event.timeMs,
            .keycode = event.keycode,
            .pressed = true,
        }))
...
void CInputManager::publishDenialKeyboardState(SP<IKeyboard> keyboard) {
    if (!keyboard || keyboard->isVirtual() || keyboard->m_xkbKeymapV1String.empty())
        return;

    auto* const router = Denial::inputRouter();
    if (!router || !router->sendFlutterKeyboardKeymap(keyboard->m_xkbKeymapV1String))
        return;

    router->sendFlutterKeyboardModifiers(Denial::SFlutterKeyboardModifiers{
...
void CInputManager::armDenialKeyboardRepeat(SP<IKeyboard> keyboard, uint32_t keycode) {
    if (!keyboard || !keyboard->m_xkbKeymap || !xkb_keymap_key_repeats(keyboard->m_xkbKeymap, keycode + 8) || keyboard->m_repeatRate <= 0 || !m_denialRepeatTimer ||
        !g_pEventLoopManager)
        return;

    m_denialRepeatKeyboard = keyboard;
    m_denialRepeatKeycode  = keycode;
    m_denialRepeatTimer->updateTimeout(std::chrono::milliseconds(std::max(1, keyboard->m_repeatDelay)));
}

void CInputManager::disarmDenialKeyboardRepeat() {
    if (m_denialRepeatTimer && g_pEventLoopManager)
        m_denialRepeatTimer->updateTimeout(std::nullopt);
    m_denialRepeatKeyboard.reset();
    m_denialRepeatKeycode = 0;
}

void CInputManager::flushDenialKeyboardOwnership() {
    disarmDenialKeyboardRepeat();

    auto* const router = Denial::inputRouter();
    const auto  timeMs = sc<uint32_t>(Time::millis(Time::steadyNow()));
    m_denialKeyboardOwnership.flush([&](uint32_t keycode) {
        return router &&
            router->sendFlutterKeyboardKey(Denial::SFlutterKeyboardEvent{
                .timeMs  = timeMs,
                .keycode = keycode,
...
void CInputManager::onKeyboardKey(const IKeyboard::SKeyEvent& event, SP<IKeyboard> pKeyboard) {
    ...
    // The lock router precedes event-bus listeners and keybindings. Physical
    // keys go only to Flutter; virtual keys are swallowed while locked.
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        if (!pKeyboard->isVirtual())
            routeDenialKeyboardKey(event, pKeyboard, true);
        return;
    }

    // Releases for keys pressed on the lock screen remain native-owned after
    // unlock. Consume them before observers and keybindings so no orphan event
    // can mutate desktop input state.
    if (!pKeyboard->isVirtual() && routeDenialKeyboardKey(event, pKeyboard, false))
        return;
    ...
    if (!DISALLOWACTION && routeDenialKeyboardKey(event, pKeyboard, passEvent))
        return;
...
bool CInputManager::unconstrainMouse() {
    if (g_pSeatManager->m_mouse.expired())
        return false;

    bool       released    = false;
    const auto constraints = m_constraints;
...
bool CInputManager::releaseMouseGrab() {
    if (!unconstrainMouse())
        return false;

    // A locked client commonly hides the cursor once and leaves that image in
    // compositor state. The user escape owns the cursor again, so replace the
    // stale client image as well as releasing the pointer constraint.
    m_cursorSurfaceInfo.wlSurface->unassign();
    m_cursorSurfaceInfo.vHotspot = {};
    ...
    Log::logger->log(Log::INFO, "Released application mouse grab and restored the default cursor");
    return true;
}

bool CInputManager::reactivateFocusedMouseGrab() {
    const auto surface    = Desktop::View::CWLSurface::fromResource(Desktop::focusState()->surface());
