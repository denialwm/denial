...
#include "DenialKeyboardOwnership.hpp"
...
enum class EDenialPointerTarget : uint8_t {
    None,
    FlutterShell,
    ClientWindow,
};
...
struct STouchData {
    ...
    PHLMONITORREF           touchFocusMonitor;
    WP<CWLSurfaceResource>  touchFocusSurface;
    Vector2D                touchSurfaceOrigin;
    int32_t                 touchFocusFlutterShellID = -1;
    bool                    touchFocusFlutterShell   = false;
    bool                    touchFocusDenial         = false;
    Vector2D                denialRectPos;
    Vector2D                denialRectSize;
    Vector2D                denialSourcePos;
    Vector2D                denialSourceSize;
};
...
class CInputManager {
  public:
    ...
    bool               unconstrainMouse();
    bool               releaseMouseGrab();
    bool               reactivateFocusedMouseGrab();
    bool               isConstrained();
    bool               isLocked();
    bool               hasHeldButtons();
    ...
    void               onDenialSessionLockChanged(bool locked);
    ...
    void               onTouchCancel(ITouch::SCancelEvent);
  ...
  private:
    ...
    bool               routeDenialKeyboardKey(const IKeyboard::SKeyEvent& event, SP<IKeyboard> keyboard, bool allowNewCapture);
    void               publishDenialKeyboardState(SP<IKeyboard> keyboard);
    void               armDenialKeyboardRepeat(SP<IKeyboard> keyboard, uint32_t keycode);
    void               disarmDenialKeyboardRepeat();
    void               flushDenialKeyboardOwnership();
    bool               routeDenialPointerMotion(uint32_t timeMs, PHLMONITOR monitor, const Vector2D& global, bool capturedOnly = false);
    bool               routeDenialPointerButton(const IPointer::SButtonEvent& e, SP<IPointer> mouse);
    bool               routeDenialPointerAxis(const IPointer::SAxisEvent& e, double factor, bool touchpadScroll);
    void               dispatchPointerAxis(const IPointer::SAxisEvent& e, double factor, wl_pointer_axis_relative_direction relativeDirection);
    // Retargets native dispatch inside the composed scene without ending the
    // pointer lifecycle observed by Flutter.
    void resetDenialPointerTarget();
    // Ends both native routing and the Flutter pointer lifecycle.
    void            clearDenialPointerTarget();
    ...
    struct {
        EDenialPointerTarget   target = EDenialPointerTarget::None;
        ...
        Vector2D               sourcePos;
        Vector2D               sourceSize;
        Vector2D               outputLogical;
        Vector2D               local;
        std::list<uint32_t>    capturedButtons;
    } m_denialPointer;
    ...
    Denial::CKeyboardOwnership m_denialKeyboardOwnership;
    SP<CEventLoopTimer>        m_denialRepeatTimer;
    WP<IKeyboard>              m_denialRepeatKeyboard;
    uint32_t                   m_denialRepeatKeycode = 0;
