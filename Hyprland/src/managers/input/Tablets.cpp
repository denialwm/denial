...
void CInputManager::onTabletAxis(CTablet::SAxisEvent e) {
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        const auto tablet = e.tablet;
        const auto tool   = ensureTabletToolPresent(e.tool);
        if (tool->m_active && (e.updatedAxes & (CTablet::eTabletToolAxes::HID_TABLET_TOOL_AXIS_X | CTablet::eTabletToolAxes::HID_TABLET_TOOL_AXIS_Y))) {
            const Vector2D delta = {
                std::isnan(e.axisDelta.x) ? 0.0 : e.axisDelta.x,
                std::isnan(e.axisDelta.y) ? 0.0 : e.axisDelta.y,
            };
            if (!std::isnan(e.axis.x))
                tool->m_absolutePos.x = e.axis.x;
            if (!std::isnan(e.axis.y))
                tool->m_absolutePos.y = e.axis.y;
            if (tablet->m_relativeInput || e.tool->type == Aquamarine::ITabletTool::AQ_TABLET_TOOL_TYPE_MOUSE)
                g_pPointerManager->move(delta);
            else
                g_pPointerManager->warpAbsolute(transformToActiveRegion(e.axis, tablet->m_activeArea), tablet);
...
void CInputManager::onTabletTip(CTablet::STipEvent e) {
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        const auto tablet = e.tablet;
        const auto tool   = ensureTabletToolPresent(e.tool);
        if (tablet->m_relativeInput)
            g_pPointerManager->move({0, 0});
        else
            g_pPointerManager->warpAbsolute(transformToActiveRegion(e.tip, tablet->m_activeArea), tablet);
        // A contact that began while locked must not become a synthetic down
        // on a client when the session is later unlocked.
        tool->m_isDown = false;
        simulateMouseMovement();
        return;
    }
...
void CInputManager::onTabletButton(CTablet::SButtonEvent e) {
    if (auto* const router = Denial::inputRouter(); router && router->secureSessionLocked()) {
        const auto tool = ensureTabletToolPresent(e.tool);
        if (!e.down)
            std::erase(tool->m_buttonsDown, e.button);
        return;
    }
