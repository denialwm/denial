...
CKeybindManager::CKeybindManager() {
    ...
    // deniald installs the input router before binds can fire. Keeping a
    // normal dispatcher here lets the Hyprland config remain the single source
    // of truth for shell shortcuts without teaching Flutter about raw keys.
    m_dispatchers["denial"] = [](std::string action) -> SDispatchResult {
        auto* const ROUTER = Denial::inputRouter();
        if (!ROUTER)
            return {.success = false, .error = "Denial input router is unavailable"};

        std::string error;
        if (!ROUTER->dispatchShortcutAction(action, error))
            return {.success = false, .error = error.empty() ? "Unsupported Denial action" : std::move(error)};
...
SDispatchResult CKeybindManager::handleKeybinds(const uint32_t modmask, const SPressedKeyWithMods& key, bool pressed, SP<IKeyboard> keyboard, SP<IHID> device) {
    ...
    bool              found = false;
    SDispatchResult   res;
    const auto* const DENIAL_ROUTER    = Denial::inputRouter();
    const bool        DENIAL_EXCLUSIVE = DENIAL_ROUTER && DENIAL_ROUTER->shellExclusiveMode();
    ...
    for (auto& k : m_keybinds) {
        ...
        const bool DENIAL_EXCLUSIVE_ALLOWED =
            (k->handler == "denial" && (k->arg == "session:lock" || k->arg == "shell:overview" || k->arg == "shell:window-switch-next" || k->arg == "shell:window-switch-end")) ||
            k->handler == "releasecursor";
        if (DENIAL_EXCLUSIVE && !DENIAL_EXCLUSIVE_ALLOWED) {
            if (SPECIALTRIGGERED && !pressed)
                std::erase_if(m_pressedSpecialBinds, [&](const auto& other) { return other == k; });
            if (!k->nonConsuming)
                found = true;
            Log::logger->log(Log::DEBUG, "Keybind suppressed by exclusive Denial shell mode handler={} arg={}", k->handler, k->arg);
            continue;
        }
...
SDispatchResult CKeybindManager::changeMouseBindMode(const eMouseBindMode MODE) {
    if (MODE != MBIND_INVALID) {
        ...
        if (auto* const ROUTER = Denial::inputRouter(); ROUTER && ROUTER->windowGeometryLocked(PWINDOW->m_stableID)) {
            Log::logger->log(Log::DEBUG, "Move/resize suppressed for shell-fullscreen window_id={}", PWINDOW->m_stableID);
