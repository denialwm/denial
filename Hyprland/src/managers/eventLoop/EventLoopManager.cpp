...
CEventLoopAsyncSignal::CEventLoopAsyncSignal(wl_event_loop* loop, void (*callback)(void*), void* data) :
    m_eventfd(eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK)), m_callback(callback), m_data(data) {
    if (m_eventfd.isValid())
        m_eventSource = wl_event_loop_add_fd(loop, m_eventfd.get(), WL_EVENT_READABLE, onReadable, this);
}

CEventLoopAsyncSignal::~CEventLoopAsyncSignal() {
    if (m_eventSource)
        wl_event_source_remove(m_eventSource);
}

bool CEventLoopAsyncSignal::signal() const {
    constexpr uint64_t VALUE = 1;
    return write(m_eventfd.get(), &VALUE, sizeof(VALUE)) == sizeof(VALUE) || errno == EAGAIN;
}

int CEventLoopAsyncSignal::onReadable(int fd, uint32_t mask, void* data) {
    (void)mask;
    auto* signal = sc<CEventLoopAsyncSignal*>(data);
    uint64_t value;
    while (read(fd, &value, sizeof(value)) == sizeof(value)) {
        ;
    }
    signal->m_callback(signal->m_data);
    return 0;
}
...
CEventLoopManager::CEventLoopManager(wl_display* display, wl_event_loop* wlEventLoop) {
    ...
    m_wake.eventfd    = CFileDescriptor{eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK)};
...
CEventLoopManager::~CEventLoopManager() {
    ...
    if (m_wake.eventSource)
        wl_event_source_remove(m_wake.eventSource);
...
static int eventLoopWakeWrite(int fd, uint32_t mask, void* data) {
    g_pEventLoopManager->onWake();
    return 0;
}
...
void CEventLoopManager::enterLoop() {
    ...
    if (m_wake.eventfd.isValid())
        m_wake.eventSource = wl_event_loop_add_fd(m_wayland.loop, m_wake.eventfd.get(), WL_EVENT_READABLE, eventLoopWakeWrite, nullptr);
    ...
    // Listener must be attached before the initial sync: if a pollFDsChanged
    // signal (e.g. a connector/session becoming ready during async DRM bind)
    // fires between these two steps, the old ordering silently drops it —
    // syncPollFDs() only runs once here and nothing else re-triggers it for
    // an already-known GPU, leaving its fd permanently unregistered from the
    // event loop for the rest of the session (TB373FU: boot-random ~1Hz
    // stall from a missed DRM page-flip-complete wakeup).
...
void CEventLoopManager::postToLoop(std::function<void()>&& fn) {
    {
        std::lock_guard<std::mutex> lock(m_wake.mutex);
        m_wake.pending.emplace_back(std::move(fn));
    }

    wakeFromAnyThread();
}

void CEventLoopManager::wakeFromAnyThread() {
    if (!m_wake.eventfd.isValid())
        return;

    uint64_t value = 1;
    if (write(m_wake.eventfd.get(), &value, sizeof(value)) < 0 && errno != EAGAIN)
        Log::logger->log(Log::ERR, "event loop wake write failed: {}", strerror(errno));
}

void CEventLoopManager::onWake() {
    if (m_wake.eventfd.isValid()) {
        uint64_t value = 0;
        while (read(m_wake.eventfd.get(), &value, sizeof(value)) == sizeof(value)) {
            ;
        }
    }
...
SP<CEventLoopAsyncSignal> CEventLoopManager::createAsyncSignal(void (*callback)(void*), void* data) {
    auto signal = SP<CEventLoopAsyncSignal>(new CEventLoopAsyncSignal(m_wayland.loop, callback, data));
    if (!signal->m_eventSource)
        return {};
    return signal;
}
