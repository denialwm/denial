...
class CWLSurfaceResource {
  public:
    ...
    uint32_t                      frame(const Time::steady_tp& now);
    uint32_t                      id();
    void                          map();
    void                          unmap();
    ...
    uint32_t                               presentFeedback(const Time::steady_tp& when, PHLMONITOR pMonitor, bool discarded = false);
    void                                   queuePresentationFeedback(PHLMONITOR pMonitor, bool discarded = false, uint64_t commitSequence = 0);
