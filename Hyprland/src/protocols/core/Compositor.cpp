...
uint32_t CWLSurfaceResource::frame(const Time::steady_tp& now) {
    if (m_current.callbacks.empty())
        return 0;

    const auto SENT = sc<uint32_t>(m_current.callbacks.size());
...
uint32_t CWLSurfaceResource::presentFeedback(const Time::steady_tp& when, PHLMONITOR pMonitor, bool discarded) {
    const auto SENT = frame(when);
    queuePresentationFeedback(pMonitor, discarded);
    return SENT;
}

void CWLSurfaceResource::queuePresentationFeedback(PHLMONITOR pMonitor, bool discarded, uint64_t commitSequence) {
    const auto SELF = m_self.lock();

    if (!PROTO::presentation || !PROTO::presentation->hasPendingFeedbackFor(SELF))
        return;

    auto FEEDBACK = makeUnique<CQueuedPresentationData>(SELF);
    FEEDBACK->attachMonitor(pMonitor);
    FEEDBACK->setCommitSequence(commitSequence);
    if (discarded)
        FEEDBACK->discarded();
    else {
        FEEDBACK->presented();
