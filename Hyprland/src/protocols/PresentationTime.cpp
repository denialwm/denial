...
void CQueuedPresentationData::setCommitSequence(uint64_t sequence) {
    m_commitSequence = sequence;
}
    ...
    for (auto const& feedback : m_feedbacks) {
        ...
        for (auto const& data : m_queue) {
            if (!data->m_surface || data->m_surface != feedback->m_surface || (data->m_monitor && data->m_monitor != pMonitor) ||
                data->m_commitSequence != feedback->m_commitSequence)
                continue;
...
void CPresentationProtocol::tagSurfaceFeedbacks(const SP<CWLSurfaceResource>& surface, uint64_t commitSequence) {
    if (!surface || commitSequence == 0)
        return;

    for (auto& feedback : m_feedbacks) {
        if (feedback->m_surface == surface && feedback->m_commitSequence == 0)
            feedback->m_commitSequence = commitSequence;
    }
}
...
bool CPresentationProtocol::hasPendingFeedbackFor(const SP<CWLSurfaceResource>& surface) const {
    return std::ranges::any_of(m_feedbacks, [&surface](const auto& feedback) { return feedback->m_surface == surface; });
}
