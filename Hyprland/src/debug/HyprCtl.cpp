...
static void appendDeniaHyprctlEvent(const std::string& message) {
...
static std::string monitorsRequest(eHyprCtlOutputFormat format, std::string request) {
    ...
    if (format == eHyprCtlOutputFormat::FORMAT_JSON) {
        ...
        for (auto const& m : allMonitors ? g_pCompositor->m_realMonitors : g_pCompositor->m_monitors) {
            if (!m || !m->m_output || m->m_id == -1) {
                appendDeniaHyprctlEvent(std::format("monitors.skip json all={} monitor={} id={} enabled={} output={} impl={} data={} strong={} monitors={} realMonitors={} focus={}",
                                                  allMonitors, m ? m->m_name : "<null>", m ? m->m_id : -999, m ? m->m_enabled : false,
                                                  m && m->m_output ? m->m_output->name : "<null>",
                                                  rc<uintptr_t>(m.impl_), rc<uintptr_t>(m.m_data), m.strongRef(),
        ...
        for (auto const& m : allMonitors ? g_pCompositor->m_realMonitors : g_pCompositor->m_monitors) {
            if (!m || !m->m_output || m->m_id == -1) {
                appendDeniaHyprctlEvent(std::format("monitors.skip normal all={} monitor={} id={} enabled={} output={} impl={} data={} strong={} monitors={} realMonitors={} focus={}",
                                                  allMonitors, m ? m->m_name : "<null>", m ? m->m_id : -999, m ? m->m_enabled : false,
                                                  m && m->m_output ? m->m_output->name : "<null>",
                                                  rc<uintptr_t>(m.impl_), rc<uintptr_t>(m.m_data), m.strongRef(),
                                                  g_pCompositor ? g_pCompositor->m_monitors.size() : 0,
...
static std::string activeWorkspaceRequest(eHyprCtlOutputFormat format, std::string request) {
    if (!Desktop::focusState()->monitor()) {
        appendDeniaHyprctlEvent(std::format("activeworkspace.focusNull monitors={} realMonitors={} unsafeState={} unsafeOutput={}",
                                          g_pCompositor ? g_pCompositor->m_monitors.size() : 0, g_pCompositor ? g_pCompositor->m_realMonitors.size() : 0,
                                          g_pCompositor ? g_pCompositor->m_unsafeState : false,
                                          g_pCompositor && g_pCompositor->m_unsafeOutput ? g_pCompositor->m_unsafeOutput->m_name : "<null>"));
        return "unsafe state";
    }
