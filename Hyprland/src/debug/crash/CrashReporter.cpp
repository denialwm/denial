...
void CrashReporter::createAndSaveCrash(int sig) {
    ...
    // destroy things as soon as possible.
    {
        ...
        const auto                         HOME       = SignalSafe::environmentValue("HOME");
        const auto                         CACHE_HOME = SignalSafe::environmentValue("XDG_CACHE_HOME");
        ...
        reportPath += "/denialCrashReport";
        reportPath.writeNum(getpid());
        reportPath += ".txt";

        {
            SignalSafe::CBufFileWriter<64> stderrOut(STDERR_FILENO);
            stderrOut += "Denial has crashed. Consult the crash report at ";
