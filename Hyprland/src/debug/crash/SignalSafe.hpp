...
namespace SignalSafe {
    ...
    // Freeze crash-report paths before signal handlers are installed. The
    // handler then reads fixed storage instead of walking `environ`.
    void        captureEnvironment();
    char const* environmentValue(const char* name);
