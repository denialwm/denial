...
namespace {
    void appendDeniaOutputEvent(const std::string& message) {
...
CMonitor::~CMonitor() {
    appendDeniaOutputEvent(std::format("~CMonitor monitor={} id={} enabled={} output={} monitors={} realMonitors={} selfValid={} selfImpl={}",
                                     m_name, m_id, m_enabled, m_output ? m_output->name : "<null>",
                                     g_pCompositor ? g_pCompositor->m_monitors.size() : 0,
                                     g_pCompositor ? g_pCompositor->m_realMonitors.size() : 0,
                                     m_self.valid(), rc<uintptr_t>(m_self.impl_)));
...
void CMonitor::onConnect(bool noRule) {
    ...
    m_listeners.presented = m_output->events.present.listen([this](const Aquamarine::IOutput::SPresentEvent& event) {
        SMonitorPresentationEvent presentation{
            .presented = event.presented,
            .sequence  = event.seq,
            .refreshNs = event.refresh,
            .flags     = event.flags,
        };
        if (event.when && event.when->tv_sec > 2)
            presentation.when = *event.when;
        else {
            clock_gettime(CLOCK_MONOTONIC, &presentation.when);
            presentation.flags &= ~Aquamarine::IOutput::AQ_OUTPUT_PRESENT_HW_CLOCK;
        }

        const auto notifyPresentationProtocol = [this, &presentation] {
            if (!PROTO::presentation->hasPendingFeedbacks())
                return;
            PROTO::presentation->onPresented(m_self.lock(), presentation.when, presentation.refreshNs, presentation.sequence, presentation.flags);
        };

        // Flutter owns the scene and frame callbacks. Keep only the physical
        // presentation protocol flush, then notify the runtime immediately.
        if (Denial::frameSink()) {
            m_events.presented.emit(presentation);
            // The runtime listener queues feedback for the exact submitted
            // scene above. Flush it against this KMS timestamp, not the next
            // output presentation.
            notifyPresentationProtocol();
            return;
        }
    ...
    m_listeners.destroy = m_output->events.destroy.listen([this] {
        appendDeniaOutputEvent(std::format("output.destroy monitor={} enabled={} output={} monitors={} realMonitors={}", m_name, m_enabled,
                                         m_output ? m_output->name : "<null>", g_pCompositor ? g_pCompositor->m_monitors.size() : 0,
                                         g_pCompositor ? g_pCompositor->m_realMonitors.size() : 0));
    ...
    appendDeniaOutputEvent(std::format("onConnect.visible monitor={} id={} output={} monitors={} realMonitors={} selfImpl={} realImpl={} realStrong={} focus={}", m_name, m_id,
                                     m_output ? m_output->name : "<null>", g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                     rc<uintptr_t>(m_self.impl_), rc<uintptr_t>(thisWrapper->impl_), thisWrapper->strongRef(),
                                     Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
...
// TB373FU bisection (2026-07-09): durable, fsync'd breadcrumb, same file as
// Compositor.cpp's tb373fuCleanupTrace (separate translation unit, so
// duplicated here rather than shared via a header) so the two interleave
// chronologically and we can tell whether this patched code path actually
// ran vs. the kernel's original deferred disable firing instead.
...
void CMonitor::onDisconnect(bool destroy) {
    tb373fuCleanupTrace("MDIS_entry onDisconnect called");
    appendDeniaOutputEvent(std::format("onDisconnect.begin monitor={} destroy={} enabled={} shuttingDown={} output={} focus={} monitors={} realMonitors={}",
                                     m_name, destroy, m_enabled, g_pCompositor->m_isShuttingDown, m_output ? m_output->name : "<null>",
                                     Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>",
                                     g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size()));
    ...
    if (!m_enabled || g_pCompositor->m_isShuttingDown) {
        appendDeniaOutputEvent(std::format("onDisconnect.skip monitor={} enabled={} shuttingDown={} monitors={} realMonitors={}", m_name, m_enabled,
                                         g_pCompositor->m_isShuttingDown, g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size()));
        tb373fuCleanupTrace("MDIS02 skip pre-shutdown disable");
        return;
    }
    ...
    appendDeniaOutputEvent(std::format("onDisconnect.backup monitor={} backup={} monitors={} realMonitors={} focus={}", m_name,
                                     BACKUPMON ? BACKUPMON->m_name : "<null>", g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                     Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
    ...
    if (BACKUPMON) {
        ...
        appendDeniaOutputEvent(std::format("onDisconnect.noBackupResetFocus monitor={} monitors={} realMonitors={}", m_name,
                                         g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size()));
    ...
    appendDeniaOutputEvent(std::format("onDisconnect.eraseMonitor monitor={} monitorsBefore={} realMonitors={} focus={}", m_name,
                                     g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                     Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
    std::erase_if(g_pCompositor->m_monitors, [&](PHLMONITOR& el) { return el.get() == this; });
    appendDeniaOutputEvent(std::format("onDisconnect.end monitor={} monitorsAfter={} realMonitors={} focus={}", m_name,
                                     g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                     Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
}
...
void CMonitor::setMirror(const std::string& mirrorOf) {
    ...
    appendDeniaOutputEvent(std::format("setMirror.begin monitor={} id={} mirrorOf={} target={} currentMirror={} monitors={} realMonitors={} focus={}",
                                     m_name, m_id, mirrorOf, PMIRRORMON ? PMIRRORMON->m_name : "<null>",
                                     m_mirrorOf ? m_mirrorOf->m_name : "<null>", g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                     Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
    ...
    if (!PMIRRORMON) {
        ...
        appendDeniaOutputEvent(std::format("setMirror.disable.visible monitor={} id={} monitors={} realMonitors={} focus={}", m_name, m_id,
                                         g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                         Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
        ...
        appendDeniaOutputEvent(std::format("setMirror.enable.eraseMonitor monitor={} id={} backup={} mirrorsTo={} monitorsBefore={} realMonitors={} focus={}",
                                         m_name, m_id, BACKUPMON ? BACKUPMON->m_name : "<null>", PMIRRORMON ? PMIRRORMON->m_name : "<null>",
                                         g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                         Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
        // remove from mvmonitors
        std::erase_if(g_pCompositor->m_monitors, [&](const auto& other) { return other == m_self; });
        appendDeniaOutputEvent(std::format("setMirror.enable.end monitor={} id={} monitorsAfter={} realMonitors={} focus={}", m_name, m_id,
                                         g_pCompositor->m_monitors.size(), g_pCompositor->m_realMonitors.size(),
                                         Desktop::focusState()->monitor() ? Desktop::focusState()->monitor()->m_name : "<null>"));
