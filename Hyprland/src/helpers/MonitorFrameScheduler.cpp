...
bool CMonitorFrameScheduler::newSchedulingEnabled() {
    ...
    if (const auto SINK = Denial::frameSink(); SINK && SINK->claimsMonitor(m_monitor.lock()))
        return false;
...
void CMonitorFrameScheduler::onFrame() {
    ...
    // deniald owns the complete output cadence and final scene. Bypass
    // Hyprland's desktop scheduling, solitary-client, tearing, animation and
    // damage bookkeeping and the desktop scheduler's logging path before any
    // of it is evaluated.
    if (const auto SINK = Denial::frameSink(); SINK && SINK->claimsMonitor(PMONITOR)) {
        if (!PMONITOR || !PMONITOR->m_enabled ||
            (g_pCompositor->m_aqBackend->hasSession() && !g_pCompositor->m_aqBackend->session->active) || !g_pCompositor->m_sessionActive || g_pCompositor->m_unsafeState)
            return;

        // The normal renderer emits this before touching the frame. Monitor
        // rule reloads (including live refresh-rate changes) are driven by
        // this event, so the Flutter-owned path must preserve it as well.
        // Applying a rule may replace this scheduler; keep only a weak guard
        // across the callback and re-fetch the sink afterwards.
        auto self = m_self;
        Event::bus()->m_events.render.preChecks.emit(PMONITOR);
        if (!self || !PMONITOR->m_enabled)
            return;

        if (const auto CURRENT_SINK = Denial::frameSink(); CURRENT_SINK)
            CURRENT_SINK->renderMonitor(PMONITOR);
        return;
    }
