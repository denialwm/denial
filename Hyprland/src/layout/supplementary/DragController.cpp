...
namespace {
    void notifyDenialPlacement(const SP<ITarget>& target, eMouseBindMode mode, Denial::EClientWindowPlacementPhase phase) {
        ...
        const auto change = mode == MBIND_MOVE ? Denial::EClientWindowPlacementChange::Move : Denial::EClientWindowPlacementChange::Resize;
        router->notifyClientWindowPlacement(target->window(), phase, change);
    }

    void setDenialDragCursorVisible(bool visible) {
        auto* const router = Denial::inputRouter();
        if (!router)
            return;
        ...
        const auto monitor  = g_pCompositor->getMonitorFromVector(position);
        if (monitor)
            router->sendFlutterPointerMove(monitor->m_id, position - monitor->m_position);
    }
}
...
void CDragStateController::dragBegin(SP<ITarget> target, eMouseBindMode mode) {
    ...
    notifyDenialPlacement(DRAGGINGTARGET, m_dragMode, Denial::EClientWindowPlacementPhase::Begin);
    setDenialDragCursorVisible(false);
}
void CDragStateController::dragEnd() {
    ...
    if (!validMapped(draggingTarget->window())) {
        ...
        setDenialDragCursorVisible(true);
        return;
    }
    ...
    notifyDenialPlacement(draggingTarget, m_dragMode, Denial::EClientWindowPlacementPhase::End);
...
void CDragStateController::mouseMove(const Vector2D& mousePos) {
    ...
    DRAGGINGTARGET->damageEntire();
    notifyDenialPlacement(DRAGGINGTARGET, m_dragMode, Denial::EClientWindowPlacementPhase::Update);
}
