...
std::expected<SGeometryRequested, eGeometryFailure> CWindowTarget::desiredGeometry() {
    ...
    bool centerManagedX11 = false;
    if (m_window->m_isX11 && Denial::inputRouter() && !m_window->isX11OverrideRedirect()) {
        // ICCCM distinguishes a requested initial position from the X
        // geometry's incidental value through USPosition/PPosition. Keep an
        // explicitly requested position, but otherwise let the floating
        // algorithm center the window on its assigned monitor.
        constexpr uint32_t POSITION_HINTS = (1U << 0U) | (1U << 2U);
        const auto         HINTS          = m_window->m_xwaylandSurface->m_sizeHints.get();
        const bool         HAS_POSITION   = HINTS && (HINTS->flags & POSITION_HINTS);

        // A box split by the assigned monitor edge is generally a toolkit
        // centering against the combined X11 screen, not a useful per-monitor
        // placement. This is notably how Steam's bootstrap window arrives.
        const auto MONITOR_BOX = PMONITOR->logicalBox();
        const CBox REQUESTED_BOX{requested.pos.value(), requested.size};
        const bool OVERLAPS_MONITOR = REQUESTED_BOX.x < MONITOR_BOX.x + MONITOR_BOX.w && REQUESTED_BOX.x + REQUESTED_BOX.w > MONITOR_BOX.x &&
            REQUESTED_BOX.y < MONITOR_BOX.y + MONITOR_BOX.h && REQUESTED_BOX.y + REQUESTED_BOX.h > MONITOR_BOX.y;
        const bool FITS_MONITOR = REQUESTED_BOX.x >= MONITOR_BOX.x && REQUESTED_BOX.y >= MONITOR_BOX.y &&
            REQUESTED_BOX.x + REQUESTED_BOX.w <= MONITOR_BOX.x + MONITOR_BOX.w && REQUESTED_BOX.y + REQUESTED_BOX.h <= MONITOR_BOX.y + MONITOR_BOX.h;

        centerManagedX11 = !HAS_POSITION || (OVERLAPS_MONITOR && !FITS_MONITOR);
        if (centerManagedX11)
            requested.pos.reset();
    }

    // TODO: detect a popup in a more consistent way.
    if ((DESIRED_GEOM.x == 0 && DESIRED_GEOM.y == 0) || !m_window->m_isX11 || centerManagedX11) {
