...
void CXWM::readProp(SP<CXWaylandSurface> XSURF, uint32_t atom, xcb_get_property_reply_t* reply) {
    ...
    auto handleWindowType = [&]() {
        ...
        const std::vector<uint32_t> atoms(atomsArr, atomsArr + reply->value_len);
        if (XSURF->m_atoms == atoms)
            return;
        XSURF->m_atoms = atoms;
        XSURF->m_events.metadataChanged.emit();
    };
    ...
    auto handleMotifHints = [&]() {
        constexpr uint32_t MOTIF_HINTS_DECORATIONS = 1U << 1U;

        bool wantsDecorations = true;
        if (reply->format == 32 && reply->value_len >= 3) {
            const auto* hints = rc<const uint32_t*>(value);
            if (hints[0] & MOTIF_HINTS_DECORATIONS)
                wantsDecorations = hints[2] != 0;
        }

        if (XSURF->m_motifWantsDecorations == wantsDecorations)
            return;

        XSURF->m_motifWantsDecorations = wantsDecorations;
        XSURF->m_events.metadataChanged.emit();
    };
    ...
    if (atom == XCB_ATOM_WM_CLASS)
        ...
        handleWMRole();
    else if (atom == HYPRATOMS["_MOTIF_WM_HINTS"])
        handleMotifHints();
...
void CXWM::readWindowData(SP<CXWaylandSurface> surf) {
    const std::array<xcb_atom_t, 10> interestingProps = {
        ...
        HYPRATOMS["WM_PROTOCOLS"],  HYPRATOMS["_MOTIF_WM_HINTS"],
    };
