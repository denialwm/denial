...
void CWindow::setMinimized(bool minimized) {
    ...
    if (minimized && Desktop::focusState()->window() == m_self) {
        // Hit testing alone is not enough here: the window may already own
        // keyboard focus, held pointer buttons, or an active pointer
        // constraint. Drop that ownership before finding the window below it.
        g_pInputManager->unconstrainMouse();
        g_pInputManager->releaseAllMouseButtons();
        Desktop::focusState()->fullWindowFocus(nullptr, FOCUS_REASON_DESKTOP_STATE_CHANGE);
        g_pInputManager->refocus();
    }

    Log::logger->log(Log::DEBUG, "Window {} minimized={}", m_self.lock(), minimized);
}
...
bool CWindow::acceptsInput() const {
    return !isHidden() && !isMinimized() && !isInputBlocked();
}
...
void CWindow::commitWindow() {
    ...
    const auto PMONITOR = m_monitor.lock();
    if (!PMONITOR || !PMONITOR->m_output || !PMONITOR->m_enabled)
        return;
...
void CWindow::unmanagedSetGeometry() {
    ...
    const auto PMONITOR       = m_monitor.lock();
    auto       LOGICALPOS     = g_pXWaylandManager->xwaylandToWaylandCoords(m_xwaylandSurface->m_geometry.pos(), PMONITOR);
    ...
    if (Denial::inputRouter() && PMONITOR) {
        // Some X11 notification clients recompute their horizontal position
        // after map using the combined X screen and jump one full monitor out
        // of view. Retain the valid mapped X anchor while still following the
        // client's vertical enter/exit animation.
        const auto MONITOR_BOX = PMONITOR->logicalBox();
        const bool CURRENT_OVERLAPS_X = POS.x < MONITOR_BOX.x + MONITOR_BOX.w && POS.x + SIZ.x > MONITOR_BOX.x;
        const bool REQUEST_OUTSIDE_X  = LOGICALPOS.x + LOGICALGEOSIZE.x <= MONITOR_BOX.x || LOGICALPOS.x >= MONITOR_BOX.x + MONITOR_BOX.w;
        if (CURRENT_OVERLAPS_X && REQUEST_OUTSIDE_X)
            LOGICALPOS.x = POS.x;
    }
