...
bool IHyprRenderer::beginExternalRender(PHLMONITOR pMonitor, CRegion& damage) {
    return beginRender(pMonitor, damage, RENDER_MODE_NORMAL, nullptr, nullptr, false);
}

SP<IFramebuffer> IHyprRenderer::externalRenderFramebuffer() const {
    return m_renderData.outFB ? m_renderData.outFB : m_renderData.currentFB;
}

uint32_t IHyprRenderer::externalRenderFramebufferId() {
    return 0;
}

void IHyprRenderer::endExternalRender(const std::function<void()>& renderingDoneCallback) {
    endRender(renderingDoneCallback);
}
...
void IHyprRenderer::damageWindow(PHLWINDOW pWindow, bool forceFull) {
    if (g_pCompositor->m_unsafeState || !pWindow)
        return;
...
void IHyprRenderer::damageMonitor(PHLMONITOR pMonitor) {
    if (g_pCompositor->m_unsafeState || !pMonitor || !pMonitor->m_output || !pMonitor->m_enabled || pMonitor->isMirror())
        return;
...
void IHyprRenderer::setCursorSurface(SP<Desktop::View::CWLSurface> surf, int hotspotX, int hotspotY, bool force) {
    ...
    if (auto* const router = Denial::inputRouter())
        router->notifyCursorShape(m_cursorHasSurface ? "surface" : "none");
