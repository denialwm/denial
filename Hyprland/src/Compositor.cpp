...
static void appendDeniaCompositorEvent(const std::string& message) {
    std::ofstream out("/tmp/denia-hypr-output-events.log", std::ios::app);
    if (!out.good())
        return;

    out << "pid=" << getpid() << " " << message << "\n";
    int frameIndex = 0;
    for (const auto& frame : getBacktrace()) {
        out << "  #" << frameIndex++ << " " << frame.desc << "\n";
        if (frameIndex >= 32)
            break;
    }
    out << "\n";
...
CCompositor::CCompositor(bool onlyConfig) : m_onlyConfigVerification(onlyConfig), m_hyprlandPID(getpid()) {
    ...
    m_hyprTempDataRoot = Env::get("XDG_RUNTIME_DIR").value_or("") + "/hypr";
    ...
    Env::set("HYPRLAND_INSTANCE_SIGNATURE", m_instanceSignature);
...
void CCompositor::initServer(std::string socketName, int socketFd) {
    ...
    // Split display/render SoCs where the KMS device has no render node
    // (e.g. vendor mediatek-drm + panthor): let the environment pick the
    // render node so EGL never initializes on the display-only device.
    if (m_drmRenderNode.fd < 0) {
        if (const auto renderNode = Env::get("HYPRLAND_RENDER_NODE")) {
            m_drmRenderNode.fd = open(renderNode->c_str(), O_RDWR | O_CLOEXEC);
            Log::logger->log(Log::WARN, "HYPRLAND_RENDER_NODE override: {} -> RENDERNODEFD {}", *renderNode, m_drmRenderNode.fd);
        }
    }
    ...
    Env::set("WAYLAND_DISPLAY", m_wlDisplaySocket);
    if (!Env::get("XDG_CURRENT_DESKTOP")) {
        Env::set("XDG_CURRENT_DESKTOP", "Denial:Hyprland");
...
void CCompositor::cleanEnvironment() {
    ...
    // in startCompositor
    Env::unset("HYPRLAND_INSTANCE_SIGNATURE");

    // in main
    Env::unset("HYPRLAND_CMD");
    Env::unset("XDG_BACKEND");
    if (m_desktopEnvSet)
        Env::unset("XDG_CURRENT_DESKTOP");
...
// TB373FU bisection (2026-07-09): durable, fsync'd breadcrumb for
// CCompositor::cleanup()'s shutdown sequence. deniald's own Log::logger
// sink lives under /run (tmpfs), wiped by the WDT reset that follows this
// crash, so it never survives to be read. Write straight to /lenovocust
// (persistent partition) instead, re-opening + fsyncing every call so a
// partial sequence still lands on disk even if the process dies mid-step.
static void tb373fuCleanupTrace(const char* step) {
    int fd = open("/lenovocust/tb373fu-denial-cleanup-trace.log", O_WRONLY | O_CREAT | O_APPEND, 0666);
    if (fd < 0)
        return;
    char            buf[256];
    struct timespec ts;
    clock_gettime(CLOCK_BOOTTIME, &ts);
    int len = snprintf(buf, sizeof(buf), "%ld.%09ld pid=%d %s\n", (long)ts.tv_sec, (long)ts.tv_nsec, (int)getpid(), step);
    if (len > 0)
...
void CCompositor::cleanup() {
    ...
    tb373fuCleanupTrace("CLEAN00 enter");
    ...
    cleanEnvironment();
    tb373fuCleanupTrace("CLEAN01 post cleanEnvironment");

    // unload all remaining plugins while the compositor is
    // still in a normal working state.
    g_pPluginSystem->unloadAllPlugins();
    tb373fuCleanupTrace("CLEAN02 post unloadAllPlugins");

    m_workspaces.clear();
    tb373fuCleanupTrace("CLEAN03 post workspaces.clear");
    m_windows.clear();
    tb373fuCleanupTrace("CLEAN04 post windows.clear");

    for (auto const& m : m_monitors) {
        g_pHyprOpenGL->destroyMonitorResources(m);
    }
    tb373fuCleanupTrace("CLEAN05 post destroyMonitorResources");

    g_pXWayland.reset();
    tb373fuCleanupTrace("CLEAN06 post xwayland.reset");

    m_monitors.clear();
    tb373fuCleanupTrace("CLEAN07 post monitors.clear");

    wl_display_destroy_clients(g_pCompositor->m_wlDisplay);
    tb373fuCleanupTrace("CLEAN08 post destroy_clients");
    removeAllSignals();
    tb373fuCleanupTrace("CLEAN09 post removeAllSignals");

    g_pInputManager.reset();
    tb373fuCleanupTrace("CLEAN10 post inputManager.reset");
    g_pDynamicPermissionManager.reset();
    g_pDecorationPositioner.reset();
    g_pCursorManager.reset();
    tb373fuCleanupTrace("CLEAN11 post cursorManager.reset");
    ...
    tb373fuCleanupTrace("CLEAN12 post hyprRenderer.reset");
    g_pProtocolManager.reset();
    tb373fuCleanupTrace("CLEAN13 post protocolManager.reset");
    g_pHyprOpenGL.reset();
    tb373fuCleanupTrace("CLEAN14 post hyprOpenGL.reset");
    ...
    tb373fuCleanupTrace("CLEAN15 post eventLoopManager.reset");
    ...
    tb373fuCleanupTrace("CLEAN16 pre aqBackend.reset");

    if (m_aqBackend)
        m_aqBackend.reset();
    tb373fuCleanupTrace("CLEAN17 post aqBackend.reset");

    if (m_critSigSource)
        wl_event_source_remove(m_critSigSource);
    tb373fuCleanupTrace("CLEAN18 pre wl_display_destroy");

    // this frees all wayland resources, including sockets
    wl_display_destroy(m_wlDisplay);
    tb373fuCleanupTrace("CLEAN19 post wl_display_destroy (should never print: process exits from here)");
}
...
void CCompositor::prepareFallbackOutput() {
    appendDeniaCompositorEvent(std::format("prepareFallbackOutput.begin monitors={} realMonitors={} unsafeOutput={}", m_monitors.size(), m_realMonitors.size(),
                                           m_unsafeOutput ? m_unsafeOutput->m_name : "<null>"));

    if (m_unsafeOutput) {
        appendDeniaCompositorEvent(std::format("prepareFallbackOutput.skip existingUnsafe monitors={} realMonitors={} unsafeOutput={}", m_monitors.size(), m_realMonitors.size(),
                                               m_unsafeOutput ? m_unsafeOutput->m_name : "<null>"));
        return;
    }

    const bool HAS_REAL_VISIBLE_MONITOR =
        std::ranges::any_of(m_monitors, [](const PHLMONITOR& monitor) { return monitor && monitor->m_output && monitor->m_enabled && !monitor->m_isUnsafeFallback; });

    if (HAS_REAL_VISIBLE_MONITOR) {
        appendDeniaCompositorEvent(std::format("prepareFallbackOutput.skip realMonitorPresent monitors={} realMonitors={}", m_monitors.size(), m_realMonitors.size()));
        return;
    }
    ...
    appendDeniaCompositorEvent(std::format("prepareFallbackOutput.afterCreate monitors={} realMonitors={} unsafeOutput={}", m_monitors.size(), m_realMonitors.size(),
                                           m_unsafeOutput ? m_unsafeOutput->m_name : "<null>"));
...
void CCompositor::startCompositor() {
    ...
    if (m_beforeEventLoopHook)
        m_beforeEventLoopHook();
...
void CCompositor::performUserChecks() {
    ...
    if (!*PNOCHECKXDG) {
        const auto currentDesktop   = Env::get("XDG_CURRENT_DESKTOP");
        const bool identifiesDenial = currentDesktop && (":" + *currentDesktop + ":").contains(":Denial:");
        if (!identifiesDenial) {
            Notification::overlay()->addNotification(I18n::i18nEngine()->localize(I18n::TXT_KEY_NOTIF_EXTERNAL_XDG_DESKTOP, {{"value", currentDesktop.value_or("unset")}}),
...
void CCompositor::openSafeModeBox() {
    ...
    box->open()->then([OPT_LOAD, OPT_OK, OPT_OPEN, this](SP<CPromiseResult<std::string>> result) {
        ...
        if (RES.starts_with(OPT_LOAD)) {
            ...
            const auto  cacheHome = Env::get("XDG_CACHE_HOME");

            if (cacheHome && !cacheHome->empty()) {
                reportPath += *cacheHome;
                reportPath += "/denial/";
            } else if (home && !home->empty()) {
                reportPath += *home;
                reportPath += "/.cache/denial/";
            }
...
void CCompositor::enterUnsafeState() {
    ...
    appendDeniaCompositorEvent(std::format("enterUnsafeState monitors={} realMonitors={} unsafeOutput={} unsafeEnabled={}", m_monitors.size(), m_realMonitors.size(),
                                           m_unsafeOutput ? m_unsafeOutput->m_name : "<null>", m_unsafeOutput ? m_unsafeOutput->m_enabled : false));
...
void CCompositor::onNewMonitor(SP<Aquamarine::IOutput> output) {
    appendDeniaCompositorEvent(std::format("onNewMonitor.begin output={} monitors={} realMonitors={}", output ? output->name : "<null>", m_monitors.size(), m_realMonitors.size()));
    ...
    appendDeniaCompositorEvent(std::format("onNewMonitor.created output={} monitor={} id={} fallback={} impl={} data={} strong={} monitors={} realMonitors={}",
                                           output ? output->name : "<null>", PNEWMONITOR ? PNEWMONITOR->m_name : "<null>", PNEWMONITOR ? PNEWMONITOR->m_id : -999, FALLBACK,
                                           rc<uintptr_t>(PNEWMONITOR.impl_), rc<uintptr_t>(PNEWMONITOR.m_data), PNEWMONITOR.strongRef(), m_monitors.size(), m_realMonitors.size()));
