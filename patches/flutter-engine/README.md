# Denial Flutter Engine patches

## Current release

The only supported patch series is under `3.44.7/`. It contains the exact
Flutter and Skia changes used to build Denial's pinned engine, generated from
the published fork branch tips and verified by `series.sha256`. See
`3.44.7/README.md` for the pinned revisions, application instructions, and
published histories, and
`prebuilt/flutter-engine/linux-x64-release/BUILD_INFO.md` for the matching
engine artifact.

For an engine upgrade, create a new versioned directory instead of changing
the 3.44.7 series. Profile the new upstream baseline first, identify which
behavior still regresses, selectively port the smallest relevant change, and
repeat the rendering, correctness, resource, and CPU checks below.
...
uses the retired Sony platform tooling or its ephemeral engine copy.
...
`0009-damage-only-marked-external-textures.patch` carries the texture IDs from
`MarkExternalTextureFrameAvailable()` through the raster thread's autonomous
draw. Reused-tree diffing then dirties only matching `TextureLayer`s, while
ordinary Dart-produced frames retain Flutter's conservative all-texture
behavior. Multiple texture notifications before one vsync are coalesced into a
set, and retry tasks preserve that set.
...
`0011-schedule-batched-external-texture-frames.patch` exposes Denial's
versioned embedder extension for publishing all texture IDs collected during
one KMS-clock transaction and scheduling exactly one frame. It also preserves
those IDs when Flutter coalesces the request with a framework-produced frame
or no cached layer tree is available. This patch is required by the
`DenialFlutterEngineScheduleFrameForExternalTextures` symbol loaded by the
Rust embedder host.
