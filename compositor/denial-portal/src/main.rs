...
mod tests {
    ...
    #[test]
    fn namespace_patterns_match_the_portal_contract() {
        assert!(namespace_matches(&[], APPEARANCE_NAMESPACE));
        assert!(namespace_matches(&[String::new()], APPEARANCE_NAMESPACE));
    ...
    #[test]
    fn portal_routing_keeps_gtk_as_the_settings_fallback() {
        let routing = include_str!("../../../packaging/arch/denial-portals.conf");
        assert!(
            routing
                .lines()
                .any(|line| line == "org.freedesktop.impl.portal.Settings=denial;gtk")
        );
    }

    #[test]
    fn read_and_read_all_expose_the_complete_appearance_state() {
        let cache = Arc::new(RwLock::new(Cache {
            snapshot: DesktopThemeSnapshot::new(7, DesktopColorSchemePreference::PreferLight),
        }));
        let interface = SettingsInterface { cache };
        ...
        assert_eq!(
            ...
            .expect("unsigned portal value"),
            2
        );
        let accent = interface
            .read(APPEARANCE_NAMESPACE, ACCENT_COLOR_KEY)
            .expect("known accent setting");
        assert_eq!(
            <(f64, f64, f64)>::try_from(Structure::try_from(accent).expect("accent structure"))
                .expect("accent tuple"),
            DesktopAccentColor::DENIAL_DEFAULT.portal_value()
        );
        let all = interface.read_all(vec!["org.freedesktop.*".to_owned()]);
        assert_eq!(
            u32::try_from(
                all[APPEARANCE_NAMESPACE][COLOR_SCHEME_KEY]
                    .try_clone()
                    .expect("clone portal value")
            )
            .expect("unsigned portal value"),
            2
        );
        assert!(all[APPEARANCE_NAMESPACE].contains_key(ACCENT_COLOR_KEY));
        assert!(matches!(
            interface.read("org.example", "missing"),
