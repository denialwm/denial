...
impl SystemCommandHandler {
    pub fn new(wayland_display: Option<OsString>, x11_display: Option<OsString>) -> Self {
        Self {
            wayland_display,
            x11_display,
...
fn launch_application(
    ...
    let mut command =
        application_command(arguments, launch_request_id, wayland_display, x11_display);
...
mod tests {
    ...
    fn builds_a_direct_process_with_denial_wayland_environment() {
        ...
        for variable in APPLICATION_ENVIRONMENT_REMOVALS
            .iter()
            .copied()
            .filter(|variable| !matches!(*variable, "DENIA_LAUNCH_REQUEST_ID" | "DISPLAY"))
        {
            assert_eq!(
