...
impl SystemControls {
    pub(super) fn new() -> io::Result<Self> {
        ...
        let audio_worker = thread::Builder::new()
            ...
            .spawn(move || run_audio_worker(audio_rx, audio_events))?;
...
impl PulseConnection {
    fn connect(api: &PulseApi) -> Result<Self, &'static str> {
...
fn adjust_pulse_level(api: &PulseApi, connection: &PulseConnection, delta: f64) -> Option<f64> {
    ...
    // SAFETY: volume provides the complete fixed-size pa_cvolume buffer and
    // the channel count came from the server's validated sink record.
    unsafe {
    ...
    // SAFETY: sink and volume remain live through the synchronous wait below.
    ...
    if target > 0.0 {
        // Match the C++ controller: raising the level also unmutes the sink.
        // SAFETY: the same serialized context operation rules apply.
        ...
        // SAFETY: sink remains live through the wait.
        ...
        // SAFETY: balances the lock acquired for the mute operation.
        unsafe { (api.mainloop_unlock)(connection.mainloop) };
        if !unmuted {
            return None;
        }
...
fn run_audio_worker(commands: Receiver<AudioCommand>, events: SyncSender<SystemControlEvent>) {
    ...
    while let Ok(command) = commands.recv() {
        let (mut delta, mut toggle_mute) = match command {
            AudioCommand::Adjust(delta) => (delta, false),
            AudioCommand::ToggleMute => (0.0, true),
        ...
        while let Ok(command) = commands.try_recv() {
            match command {
                ...
                AudioCommand::ToggleMute => toggle_mute = !toggle_mute,
                AudioCommand::Stop => {
        ...
        if delta == 0.0 && !toggle_mute {
            continue;
        }
        if connection.is_none() {
            match PulseConnection::connect(&api) {
        ...
        let result = if delta != 0.0 {
            adjust_pulse_level(&api, active, delta)
        } else {
            Some(0.0)
        };
        let result = result.and_then(|level| {
            if toggle_mute {
                toggle_pulse_mute(&api, active)
            } else {
                Some(level)
            }
        });
        match result {
            Some(level) => {
                ...
                let _ = events.try_send(SystemControlEvent::AudioLevel(level));
            }
            None => {
                if !std::mem::replace(&mut failure_latched, true) {
                    warn!("native PulseAudio volume adjustment failed; reconnecting");
                }
                if let Some(mut active) = connection.take() {
                    active.close(&api);
                }
...
fn receive_brightness_batch(
...
) -> Option<HashMap<String, (i64, f64)>> {
    let BrightnessCommand::Adjust {
        connector,
        monitor_id,
        delta,
    } = first
    ...
    let mut pending = HashMap::from([(connector, (monitor_id, delta))]);
    let deadline = Instant::now() + DDC_COALESCE_WINDOW;
    loop {
        let timeout = deadline.saturating_duration_since(Instant::now());
        match commands.recv_timeout(timeout) {
            Ok(BrightnessCommand::Adjust {
                connector,
                monitor_id,
                delta,
            }) => {
                let entry = pending.entry(connector).or_insert((monitor_id, 0.0));
                entry.0 = monitor_id;
                entry.1 = (entry.1 + delta).clamp(-1.0, 1.0);
            }
            Ok(BrightnessCommand::Stop) | Err(RecvTimeoutError::Disconnected) => return None,
...
fn run_brightness_worker(
    ...
    while let Ok(first) = commands.recv() {
        ...
        for (connector, (monitor_id, delta)) in batch {
