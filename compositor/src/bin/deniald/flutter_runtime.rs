    ...
    pub fn handle(&mut self, event: &SmithayInputEvent<LibinputInputBackend>) {
        match event {
            ...
            SmithayInputEvent::PointerAxis { event, .. } => {
                self.ensure_mouse_added();
                let scroll_x = event.amount(Axis::Horizontal).unwrap_or_else(|| {
                    event.amount_v120(Axis::Horizontal).unwrap_or(0.0) * 15.0 / 120.0
                });
                let scroll_y = event.amount(Axis::Vertical).unwrap_or_else(|| {
...
impl FlutterGlHandler {
    ...
    fn new<'a>(
        ...
        info!("creating stencil-backed direct Flutter atlas targets");
        let mut targets = Vec::new();

        for dmabuf in dmabufs {
            let image = match render_context.display().create_image_from_dmabuf(dmabuf) {
                Ok(image) => image,
                Err(error) => {
            ...
            unsafe {
                ...
                (gl.gen_renderbuffers)(1, &mut target.stencil_renderbuffer);
                (gl.bind_renderbuffer)(gl::RENDERBUFFER, target.stencil_renderbuffer);
            ...
            // newly created framebuffer is still bound, and both output
            // pointers reference live local integers.
            let framebuffer_status = unsafe {
                let status = (gl.check_framebuffer_status)(gl::FRAMEBUFFER);
                (gl.get_integer_v)(gl::SAMPLES, &mut actual_samples);
                (gl.get_integer_v)(gl::STENCIL_BITS, &mut stencil_bits);
                status
            };
            if target.texture == 0
                || target.framebuffer == 0
                || target.stencil_renderbuffer == 0
                ...
                || stencil_bits != 8
            {
                warn!(
                    ...
                    actual_samples,
                    stencil_bits,
                    "Flutter direct stencil atlas FBO creation failed"
                );
                let mut failed = vec![target];
                destroy_targets(gl, &display, &mut failed);
                destroy_targets(gl, &display, &mut targets);
                render_context.unbind()?;
                return Err("a Flutter direct stencil atlas framebuffer is incomplete".into());
            }
            targets.push(target);
        }
        ...
        Ok(Arc::new(Self {
            ...
            size,
            use_native_fence,
            producer_state: AtomicU8::new(FlutterProducerState::Idle.as_u8()),
        }))
    }
    ...
    fn begin_raster_frame(&self) {
        let _ = self.producer_state.compare_exchange(
            FlutterProducerState::Requested.as_u8(),
...
impl OpenGlHandler for FlutterGlHandler {
    ...
    fn present(&self, frame: PresentFrame<'_>) -> bool {
        ...
        // sealed the frame. A new physical edge cannot consume another baton
...
fn destroy_targets(gl: GlApi, display: &EGLDisplayHandle, targets: &mut Vec<GlTarget>) {
    for target in targets.drain(..).rev() {
        ...
        unsafe {
            if target.stencil_renderbuffer != 0 {
                (gl.delete_renderbuffers)(1, &target.stencil_renderbuffer);
            }
            ...
            if target.image != 0 {
                egl_ffi::egl::DestroyImageKHR(
                    display.handle,
                    target.image as egl_ffi::egl::types::EGLImageKHR,
                );
            }
        }
    }
...
impl FlutterRuntime {
    ...
    pub fn process_input(&mut self, input: &mut InputQueue) -> Result<(), Box<dyn Error>> {
        ...
        pointer_events.reserve(input.events.len());
        let mut key_message = mem::take(&mut self.key_event_scratch);
        while let Some(event) = input.events.pop_front() {
            match event {
            ...
            match event {
                ...
                RuntimeEvent::Engine {
                    ...
                    if !self.physical_vsync {
                        ...
                        if let Err(error) = self.latch_external_textures() {
                        ...
                        // A queued wakeup can outlive the baton if another
                        // physical pulse drained it first. Re-read the queue
                        // once per wakeup instead of arming a spurious pulse;
                        // the idle main-loop path remains lock-free.
                        self.physical_vsync_pending = self.handler.has_pending_vsync();
                    }
                }
                RuntimeEvent::Engine {
                    generation,
    ...
    pub fn has_pending_vsync(&self) -> bool {
        self.physical_vsync && self.physical_vsync_pending
    }

    pub fn deferred_physical_vsyncs(&self) -> u64 {
        self.deferred_physical_vsyncs
    }

    fn latch_external_textures(&mut self) -> Result<(), Box<dyn Error>> {
        self.handler
            .advance_external_texture_sources(&mut self.pending_texture_marks);
        let engine = self
            .host
            .as_ref()
            .expect("Flutter runtime is shutting down")
        ...
        while let Some(texture_id) = self.pending_texture_marks.get(marked).copied() {
            if let Err(error) = engine.mark_external_texture_frame_available(texture_id) {
                self.pending_texture_marks.drain(..marked);
                return Err(error.into());
            }
            marked += 1;
        }
        self.pending_texture_marks.clear();
        Ok(())
    }

    pub fn deliver_physical_vsync(
        &mut self,
        interval: Duration,
        observed_at: Instant,
        ...
        if !self.physical_vsync {
            return Err("physical Flutter vsync is not enabled".into());
        }
        if !self.physical_vsync_pending {
            return Ok(false);
        }
        // Mirror the C++ producer arbiter: a physical edge may retire output
        // work while Flutter is busy, but it cannot consume the next baton
        // until the preceding transaction has reached IDLE.
        if !self.handler.try_request_frame() {
            self.deferred_physical_vsyncs = self.deferred_physical_vsyncs.saturating_add(1);
            return Ok(false);
        }
        let (baton, has_more) = self.handler.take_next_vsync();
        let Some(baton) = baton else {
            self.handler.cancel_requested_frame();
            self.physical_vsync_pending = false;
            return Ok(false);
        };
        self.physical_vsync_pending = has_more;
        if let Err(error) = self.latch_external_textures() {
            self.handler.restore_vsync(baton);
            self.handler.cancel_requested_frame();
            self.physical_vsync_pending = true;
            return Err(error);
        }
        let engine = self
            .host
        ...
        let observation_delay = Instant::now().saturating_duration_since(observed_at);
        let (frame_start_nanos, frame_target_nanos) =
            physical_vsync_timestamps(now_nanos, observation_delay, presented_at, interval);
    ...
    pub fn sync_wayland_scene(
        ...
        self.handler
            .set_external_texture_sources(frames.drain(..), &mut self.pending_texture_marks);
        ...
        // Publish metadata first. Dart applies this update synchronously on
        // its platform channel before the marks below schedule texture-only
        // raster frames, keeping geometry and buffer content in one ordered
        // transaction.
        let engine = self
            .host
            .as_ref()
            .expect("Flutter runtime is shutting down")
            .engine();
        let (update, windows) = self.wire.update_windows(windows)?;
        let window_snapshot_changed = if let Some(update) = update {
        ...
        for texture_id in removed.drain(..) {
            engine.unregister_external_texture(texture_id)?;
            self.handler.remove_external_texture_source(texture_id);
            self.pending_texture_marks
                .retain(|pending| *pending != texture_id);
            self.registered_external_textures.remove(&texture_id);
        }

        // Match C++ admission semantics: notifying Flutter here creates its
        // AwaitVSync demand, while the queued source itself remains latched to
        // the next physical KMS edge. Deferring both operations to that edge
        // creates a circular wait whenever the shell is otherwise static.
        let mut marked = 0usize;
        while let Some(texture_id) = self.pending_texture_marks.get(marked).copied() {
            if let Err(error) = engine.mark_external_texture_frame_available(texture_id) {
                self.pending_texture_marks.drain(..marked);
...
mod tests {
    ...
    fn quad_buffer_mailbox_reuse_preserves_only_missing_damage() {
        ...
        assert_eq!(broker.acquire_for_render(), Some(4));
        assert_eq!(broker.acquire_for_render(), Some(4));
        let after_abandon = broker_damage(&broker, 4);
