...
impl OutputPresentationBatch {
    ...
    pub(super) fn submit_window(&mut self, output: &Output, window: &Window) {
        ...
        // not drained here: like the C++ runtime, the physical page-flip edge
        // releases that scheduling hint for the client's next transaction.
...
/// *next* frame and is dispatched from the physical page-flip path.
...
/// Advance every outstanding client frame callback on one physical output.
/// The returned count is also the C++ scheduler's lookahead condition: if a
/// client was released to produce work, queue exactly one more physical edge
/// even when the new buffer has not arrived yet.
...
/// Whether a mapped window tree currently owns a one-shot frame callback.
///
/// This is intentionally separate from sending the callback. A callback
/// committed after the compositor's one-frame lookahead has already retired
/// must be able to wake an otherwise idle output; it is completed only by the
/// physical edge which that wake queues.
#[cfg(feature = "flutter")]
pub(super) fn window_has_frame_callbacks(window: &Window) -> bool {
    let Some(root) = window.wl_surface() else {
        return false;
    };
    if surface_tree_has_frame_callbacks(&root) {
        return true;
    }
