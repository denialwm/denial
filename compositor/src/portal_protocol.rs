...
mod tests {
    ...
    fn hello_round_trips() {
        let bytes = encode_client_message(ClientMessage::Hello);
        assert_eq!(decode_client_message(&bytes), Ok(ClientMessage::Hello));
    }

    #[test]
    fn every_theme_preference_round_trips() {
        for preference in [
            DesktopColorSchemePreference::NoPreference,
            DesktopColorSchemePreference::PreferDark,
            DesktopColorSchemePreference::PreferLight,
        ] {
            let snapshot = DesktopThemeSnapshot::new(42, preference)
            ...
            assert_eq!(
                ...
                Ok(ServerMessage::ThemeSnapshot(snapshot))
            );
        }
    }

    #[test]
    fn rejects_inconsistent_derived_values() {
        let snapshot = DesktopThemeSnapshot::new(7, DesktopColorSchemePreference::PreferDark);
        let mut bytes = encode_server_message(ServerMessage::ThemeSnapshot(snapshot));
        bytes[HEADER_BYTES + 10..HEADER_BYTES + 14].copy_from_slice(&2u32.to_le_bytes());
        assert_eq!(
            decode_server_message(&bytes),
            Err(ProtocolError::InconsistentSnapshot)
        );
    }
}
