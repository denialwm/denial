...
impl fmt::Display for LoadError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Symbol(source) => write!(
                formatter,
                "FlutterEngineGetProcAddresses is unavailable: {source}"
            ),
...
mod tests {
    ...
    fn loads_the_bundled_flutter_engine_abi() {
        ...
        let candidates = [
            repository.join("dart_shell/build/elinux/x64/release/bundle/lib/libflutter_engine.so"),
            repository.join("dart_shell/elinux/flutter/ephemeral/libflutter_engine.so"),
        ];
        let engine = candidates
            .iter()
            .find(|path| path.is_file())
            .expect("repository has no Flutter engine bundle");
        let library = EngineLibrary::load(engine).expect("load bundled Flutter engine");
        assert!(library.runs_aot_compiled_dart_code());
        let library = Arc::new(library);
        let app = repository.join("dart_shell/build/elinux/x64/release/bundle/lib/libapp.so");
