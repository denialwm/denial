...
The current channels are documented in `protocol/CHANNEL_INVENTORY.md` and the
ordered wire format in `protocol/WIRE_FORMAT.md`.
...
The removed Hyprland implementation is available only through the
`hyprland-last-known-good` tag, documented in `LEGACY_HYPRLAND.md`.
