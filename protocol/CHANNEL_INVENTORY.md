...
| `denial/system_command` | Dart → native | bounded length-prefixed packet | Launch application, toggle OSK, or take screenshot |
...
| Offset | Bytes | Field |
| ---: | ---: | --- |
| 0 | 1 | command: launch `0`, toggle keyboard `1`, screenshot `2` |
...
launch accepts arguments or a request ID. Native reconstructs the command with
POSIX single-quote escaping and starts it through Hyprland's executor on the
compositor event loop; embedded Dart never invokes `Process.run` or
`Process.start`.
